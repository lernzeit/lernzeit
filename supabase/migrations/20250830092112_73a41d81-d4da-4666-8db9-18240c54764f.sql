-- Phase 3: Delete generated_templates table and clean up foreign keys (SIMPLIFIED)
...
-- Add useful indexes to the templates table for better performance (using actual columns)
...
CREATE INDEX IF NOT EXISTS idx_templates_status_active 
ON templates(status) 
WHERE status = 'ACTIVE';
