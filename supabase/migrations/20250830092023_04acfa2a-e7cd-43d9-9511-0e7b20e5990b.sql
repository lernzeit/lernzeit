-- Phase 3: Delete generated_templates table and clean up foreign keys (CORRECTED)
...
-- Note: Using correct_rate_bayes instead of qscore (actual column in templates table)
CREATE INDEX IF NOT EXISTS idx_templates_status_quality 
ON templates(status, correct_rate_bayes) 
WHERE status = 'ACTIVE';
