-- Fix Security Definer View für template_scores
...
SELECT 
  ...
  t.rating_sum,
  CASE 
    WHEN t.plays > 0 THEN ROUND((t.correct::float / t.plays::float) * 100, 2)
    ELSE 0
  END as success_rate,
  CASE 
    WHEN t.rating_count > 0 THEN ROUND(t.rating_sum::float / t.rating_count::float, 2)
    ELSE 0
