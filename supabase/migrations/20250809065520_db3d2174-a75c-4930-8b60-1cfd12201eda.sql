-- Phase 3 (fix): Use text for category to avoid missing enum
...
-- Create policies for user difficulty profiles
CREATE POLICY IF NOT EXISTS "Users can view their own difficulty profiles" 
ON public.user_difficulty_profiles 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own difficulty profiles" 
ON public.user_difficulty_profiles 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update their own difficulty profiles" 
ON public.user_difficulty_profiles 
FOR UPDATE 
USING (auth.uid() = user_id);
...
-- Create policies for question quality metrics
CREATE POLICY IF NOT EXISTS "Users can view their own quality metrics" 
ON public.question_quality_metrics 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert their own quality metrics" 
ON public.question_quality_metrics 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);
